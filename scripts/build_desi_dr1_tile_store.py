...
def main() -> int:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=PROJECT_ROOT / "data" / "raw" / "desi-dr1",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "data" / "processed" / "desi-dr1",
    )
    ...
    try:
        for path in files:
            ...
            for chunk_number, chunk in enumerate(
                iter_fits_chunks(path, args.chunk_rows),
                start=1,
            ):
                raw_rows += len(chunk)
                per_file["raw_rows"] += len(chunk)
                frame, _mapping, descriptor = build_desi_frame(
                    chunk,
                    component=component,
                ...
                writer.ingest(frame)
                accepted += len(frame)
                per_file["accepted_rows"] += len(frame)
                print(
                    f"{path.name} chunk {chunk_number}: "
                    f"read {per_file['raw_rows']:,}; "
                    f"accepted {per_file['accepted_rows']:,}"
                )
