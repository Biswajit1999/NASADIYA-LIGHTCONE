...
def stable_object_hashes(values: Iterable[object]) -> np.ndarray:
    """Vector-like wrapper around the immutable object-ID hash contract.

    This intentionally favours an auditable, implementation-stable hash over a
    process-randomised Python ``hash``.  It is suitable for streamed catalogue
    construction and deterministic validation, although it is not a replacement
    for a survey-selection function.
    """
...
def select_lowest_hash(
    ...
    *,
    ...
    """Select ``n_rows`` real rows using the lowest stable object-ID hashes.

    The returned frame is ordered by object ID so equivalent parent catalogues
    yield byte-stable table ordering independent of input/chunk order.
    """
...
def select_seeded_random(
    ...
    *,
    ...
    """Select a reproducible pseudo-random baseline with no replacement."""
...
def largest_remainder_quotas(counts: pd.Series, n_rows: int) -> pd.Series:
    """Allocate an integer point budget proportionally using largest remainder.

    Counts must be a positive, indexed series.  Ties are resolved by the string
    form of each stratum label, so the allocation remains deterministic.
    """
...
def select_stratified_lowest_hash(
    ...
    *,
    ...
    Strata are part of the representation policy, not an observational selection
    correction.  Typical use is ``("tracer",)`` or
    ``("tracer", "sky_cell", "redshift_bin")`` after those columns have been
...
def sampling_metadata(
    ...
    *,
    ...
    """Build manifest metadata after validating a selected browser representation."""
