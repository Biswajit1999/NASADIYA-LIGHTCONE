...
export class GuidedFlyby {
  constructor(scene) {
    ...
    scene.controls.rotateSpeed = 0.62;
    scene.controls.zoomSpeed = 0.88;
    scene.controls.panSpeed = 0.68;
    scene.controls.screenSpacePanning = true;
    scene.controls.addEventListener('start', () => {
      scene.focusAnimation = null;
      this.stop('manual');
    });
  }
  ...
  start() {
    ...
    this.scene.focusAnimation = null;
    ...
    const targets = [new THREE.Vector3(), new THREE.Vector3(radius * 0.10, 0, radius * 0.03), new THREE.Vector3(radius * 0.16, radius * 0.02, radius * 0.10), new THREE.Vector3(-radius * 0.12, 0, -radius * 0.10), new THREE.Vector3()];
    this.route = { startedAt: performance.now(), duration: 22000, positions: new THREE.CatmullRomCurve3(positions, false, 'centripetal'), targets: new THREE.CatmullRomCurve3(targets, false, 'centripetal') };
    this.active = true;
    this.emit({ active: true, phase: 'overview', progress: 0 });
  }

  stop(reason = 'stopped') { if (!this.active) return; this.active = false; this.route = null; this.emit({ active: false, reason, progress: 0 }); }
  ...
  tick(now) {
    ...
    if (now - this.lastUpdate > 120) { this.lastUpdate = now; this.emit({ active: true, phase: phase(progress), progress }); }
  }
}
