...
import { SceneAnnotations } from '../ui/scene-annotations.js';
...
function createObserverMarker() {
  ...
  const core = new THREE.Mesh(new THREE.SphereGeometry(4.2, 20, 20), new THREE.MeshBasicMaterial({ color: 0xf7fbff }));
  const halo = new THREE.Mesh(new THREE.RingGeometry(12, 13, 64), new THREE.MeshBasicMaterial({ color: 0x72e7ff, transparent: true, opacity: 0.52, side: THREE.DoubleSide, depthWrite: false }));
  const outer = new THREE.Mesh(new THREE.RingGeometry(26, 27, 64), new THREE.MeshBasicMaterial({ color: 0x72e7ff, transparent: true, opacity: 0.13, side: THREE.DoubleSide, depthWrite: false }));
...
function createSelectionMarker() {
  const group = new THREE.Group();
  const ring = new THREE.Mesh(new THREE.RingGeometry(10, 12.5, 72), new THREE.MeshBasicMaterial({ color: 0xeab276, transparent: true, opacity: 0.92, side: THREE.DoubleSide, depthWrite: false }));
  const core = new THREE.Mesh(new THREE.SphereGeometry(2.7, 16, 16), new THREE.MeshBasicMaterial({ color: 0xfff6eb, transparent: true, opacity: 0.95 }));
  group.add(ring, core);
  group.visible = false;
  return group;
}
...
export class LightconeScene {
  constructor(canvas) {
    ...
    this.controls.addEventListener('change', () => this.cameraChangeListeners.forEach((listener) => listener(this.camera, this.controls)));
    this.controls.addEventListener('start', () => { this.focusAnimation = null; });
    ...
    this.annotations = new SceneAnnotations(document.querySelector('.viewport-shell'), canvas);
    this.annotations.setContext({ extentMpc: this.datasetMaxDistanceMpc, mode: this.mode });
    window.addEventListener('resize', () => this.resize());
  }

  onCameraChange(listener) { this.cameraChangeListeners.add(listener); return () => this.cameraChangeListeners.delete(listener); }
  ...
  defaultFrame(mode = this.mode) {
    if (this.datasetMaxDistanceMpc <= 380) return mode === 'lightcone' ? { position: LIGHTCONE_CAMERA.clone(), target: new THREE.Vector3() } : { position: SLICE_CAMERA.clone(), target: new THREE.Vector3() };
  ...
  setAnnotationsVisible(visible) { this.annotations?.setVisible(visible); }

  animateCamera(position, target, duration = 720) {
    this.focusAnimation = { startedAt: performance.now(), duration, fromPosition: this.camera.position.clone(), toPosition: position.clone(), fromTarget: this.controls.target.clone(), toTarget: target.clone() };
  }
  ...
  focusOn(position, scale = 80) {
    ...
    direction.normalize();
    this.animateCamera(position.clone().add(direction.multiplyScalar(THREE.MathUtils.clamp(scale * 2.35 + 70, 120, 1300))), position, 800);
  }

  setSelection(position = null) {
    if (!position) { this.selectionMarker.visible = false; return; }
  ...
  tick(now) {
    ...
    if (this.observer.visible) {
      ...
      this.observer.children[2].scale.setScalar(1 + Math.sin(elapsed * 2.4) * 0.04);
    }
    if (this.selectionMarker.visible) {
      this.selectionMarker.children[0].rotation.z += 0.015;
      this.selectionMarker.scale.setScalar(1 + Math.sin(elapsed * 3.4) * 0.06);
      this.selectionMarker.lookAt(this.camera.position);
    }
    this.controls.update();
    this.annotations?.update(this.camera);
