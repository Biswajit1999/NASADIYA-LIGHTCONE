...
import { SceneHud } from './ui/scene-hud.js';

const state = {
  layerId: '2mrs', maxRedshift: SURVEY_LAYERS['2mrs'].defaultMaxRedshift, pointBudget: SURVEY_LAYERS['2mrs'].defaultPointBudget,
  sliceThickness: 24, sliceOffset: 0, showGalaxies: true, spatialMode: SURVEY_LAYERS['2mrs'].defaultSpatialMode,
  viewMode: 'catalog', tileStreaming: false, fullCatalogue: false,
...
const sceneHud = new SceneHud(document.querySelector('#explorer'));
const flyby = new GuidedFlyby(scene);
const raycaster = new THREE.Raycaster();
raycaster.params.Points.threshold = 9;
const pointer = new THREE.Vector2();

sceneHud.bind({ onFlybyToggle: () => flyby.toggle() });
flyby.onChange((status) => sceneHud.setFlyby(status));
...
function supportsFullCloud(layer = currentLayer()) { return ['desi-dr1', 'all-live'].includes(layer?.id); }

function updateSpatialAids() {
  referenceFrame.setExtent(scene.datasetMaxDistanceMpc);
  referenceFrame.setVisible(state.spatialMode === 'lightcone');
}
...
function applyState({ scheduleTiles = true } = {}) {
  ...
  sceneHud.update({ metrics, state, layer: currentLayer() });
...
function tileStreamingConfig() { return TILE_STREAMING[state.layerId] || null; }
...
async function refreshAdaptiveTiles() {
  ...
  try {
    ...
    if (result.changed && result.objects?.length) { replacePoints(result.objects, activeMeta); applyState({ scheduleTiles: false }); }
    ...
    ui.setTileStreamingStatus(delivery); highDensityControls.setDelivery(delivery);
    replacePoints(overviewObjects, activeMeta); applyState({ scheduleTiles: false });
  } finally { tileRefreshBusy = false; }
}

async function configureFullCloud(layer, requestId) {
  fullCloudProbe = null;
  if (!supportsFullCloud(layer)) {
    highDensityControls.setFullCatalogueStatus({ available: false, reason: 'Full observed-row mode is available for DESI and the 2MRS + DESI stack.' });
    return;
  }
  ...
  highDensityControls.setFullCatalogueStatus({ available: probe.available, recordCount: Number(probe.manifest?.record_count || 0), reason: probe.reason, delivery: probe.delivery });
}

async function activateFullCloud() {
  if (!supportsFullCloud() || !fullCloudProbe?.available || fullCloudLoading) return;
  if (state.fullCatalogue) { state.pointBudget = fullCloudProbe.manifest.record_count; applyState({ scheduleTiles: false }); return; }
  fullCloudLoading = true;
  ui.setLoadingState({
    title: state.layerId === 'all-live' ? 'Loading the full observed-survey stack' : 'Loading the full DESI observed cloud',
    copy: state.layerId === 'all-live'
      ? 'Keeping the 2MRS nearby anchor while streaming every accepted DESI source row into GPU memory.'
      : 'Streaming every accepted DESI source row into GPU memory. Source inspection remains available through the provenance tile view.',
    dataset: activeMeta?.dataset_label || 'DESI DR1 LSS', count: `${Number(fullCloudProbe.manifest.record_count).toLocaleString('en-GB')} DESI observed rows`, progress: 35,
  });
  try {
    const buffer = await fetchGpuCloud(fullCloudProbe);
    if (!supportsFullCloud() || state.layerId !== currentLayer().id) return;
    window.clearTimeout(tileRefreshTimer); tileStreamer = null; state.tileStreaming = false;
    ...
    ui.hideLoading();
  } finally { fullCloudLoading = false; }
}
...
async function loadSingleLayerPayload(layer) { return layer.dataKind === 'catalog' ? loadCatalog(layer.dataUrl, layer.label) : loadTileStoreOverview(layer.dataUrl, layer.label); }
function annotateSourceLayer(objects, layerId) { return objects.map((object) => ({ ...object, source_layer: layerId })); }
...
async function loadCompositeLayer(layer) {
  ...
  return {
    meta: {
      dataset_id: 'all-live-observed-surveys', dataset_label: `2MRS + DESI DR1 LSS · ${sourceRowCount.toLocaleString('en-GB')} source rows (non-deduplicated)`, object_count: sourceRowCount, overview_count: objects.length,
      source_survey: '2MRS + DESI DR1 LSS', source_release: 'Huchra et al. 2012 + DESI DR1', source_url: 'docs/sources.md', citation_key: 'Huchra2012_2MRS;DESI_DR1', measurement_kind: 'spectroscopic', object_type: 'galaxies and quasars',
      distance_note: 'Non-deduplicated comparison stack. Individual records retain their original survey provenance and cosmological placement note.', radial_uncertainty_required: false, is_synthetic: false, composite: true,
      ...
      tracer_counts: desi?.meta?.tracer_counts || {}, tile_count: Number(desi?.meta?.tile_count || 0), tile_manifest: desi?.meta?.tile_manifest || null, tile_index_url: desi?.meta?.tile_index_url || null,
    }, objects,
  };
}

async function loadLayerPayload(layer) { return layer.dataKind === 'composite' ? loadCompositeLayer(layer) : loadSingleLayerPayload(layer); }
function loadingText(layer) {
  ...
  return { title: layer.dataKind === 'tile-store' ? `Opening ${layer.label.split(' · ')[0]}` : 'Opening the observed local Universe', copy: layer.dataKind === 'tile-store' ? 'Reading the tile-store index and deterministic overview of real observed source rows…' : 'Reading the 2MRS browser catalogue and its source provenance…' };
}

async function configureAdaptiveTiles(layer, meta, requestId) {
  tileStreamer = null; state.tileStreaming = false;
  ...
  if (!config?.enabled || !meta.tile_manifest || !meta.tile_index_url || !meta.tile_count) {
    ...
    ui.setTileStreamingStatus(delivery); highDensityControls.setDelivery(delivery); return;
  }
  const streamer = new TileStreamer({ manifest: meta.tile_manifest, indexUrl: meta.tile_index_url, overviewObjects, remoteBaseUrl: config.remoteBaseUrl, maxTiles: config.maxTiles, maxCachedTiles: config.maxCachedTiles, maxLoadedRows: config.maxLoadedRows, loadConcurrency: config.loadConcurrency, overviewReserveRows: config.overviewReserveRows, overviewReserveFraction: config.overviewReserveFraction, minOverviewRows: config.minOverviewRows });
  const delivery = await streamer.probeDelivery();
  if (requestId !== loadSequence || state.layerId !== layer.id) return;
  tileStreamer = streamer; state.tileStreaming = delivery.available;
...
async function activateLayer(layerId, { initial = false } = {}) {
  const requestedLayer = SURVEY_LAYERS[layerId]; const previousLayer = currentLayer();
  if (!requestedLayer || requestedLayer.installed === false) return;
  flyby.stop('layer-change');
  const requestId = ++loadSequence;
  window.clearTimeout(tileRefreshTimer); tileStreamer = null; state.tileStreaming = false; state.fullCatalogue = false; fullCloudProbe = null;
  ...
  try {
    ...
    overviewObjects = objects; activeMeta = meta; state.layerId = requestedLayer.id;
    ...
    state.spatialMode = requestedLayer.defaultSpatialMode; state.sliceThickness = requestedLayer.supportsSlice ? 24 : state.sliceThickness; state.sliceOffset = requestedLayer.supportsSlice ? 0 : state.sliceOffset;
    replacePoints(overviewObjects, meta); scene.setSpatialMode(state.spatialMode, { immediate: true });
    ui.setDataReady(meta, maxField(objects, 'redshift'), state, requestedLayer);
    highDensityControls.configure({ layer: requestedLayer, state }); applyState({ scheduleTiles: false });
    document.title = `NĀSADĪYA LIGHTCONE — ${meta.source_survey || requestedLayer.id}`;
    await configureAdaptiveTiles(requestedLayer, meta, requestId); await configureFullCloud(requestedLayer, requestId);
  } catch (error) {
    if (requestId !== loadSequence) return;
    if (initial) { ui.showLoadError(error.message || 'Unknown catalogue-loading problem.'); return; }
    ui.hideLoading(); ui.showLayerUnavailable(requestedLayer, previousLayer);
  }
}

function initialise() {
  ui.bind({
    getState: () => state, onStateChange: () => applyState(),
    onSpatialModeChange: (mode) => { flyby.stop('mode-change'); scene.setSpatialMode(mode); scene.setSelection(null); updateSpatialAids(); scheduleAdaptiveTileRefresh(0); },
    onReset: () => { flyby.stop('reset'); scene.resetView(); scene.setSelection(null); scheduleAdaptiveTileRefresh(0); },
    onFocusLocal: () => { flyby.stop('local'); scene.setSpatialMode('slice'); scene.setSelection(null); updateSpatialAids(); scheduleAdaptiveTileRefresh(0); },
    onCloseSelection: () => scene.setSelection(null), onLayerChange: (layerId) => activateLayer(layerId),
  });
  ...
  canvas.addEventListener('pointerdown', (event) => { flyby.stop('manual'); pointerStart = { x: event.clientX, y: event.clientY }; });
  canvas.addEventListener('pointerup', (event) => { if (!pointerStart) return; if (Math.hypot(event.clientX - pointerStart.x, event.clientY - pointerStart.y) < 5) selectAtPointer(event); pointerStart = null; });
  const animate = (now) => { flyby.tick(now); points?.updateTime(now * 0.001); scene.tick(now); requestAnimationFrame(animate); };
